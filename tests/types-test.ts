...
namespace TestNonFunctionKeys {
    ...
    type TestSourceNonFunctionKeys = NonFunctionKeys<
        InstanceType<typeof TestSource>
    >;
...
namespace TestExtraPropHandler {
    ...
    const handler: ExtraPropHandler<TContext, typeof TestSource> = (
        parent,
        object,
    ) => {
...
namespace TestAttachStrategy {
    ...
    const strategy1: AttachStrategy<TContext, typeof TestSource> = (
        parent,
        child,
    ) => {
        return () => {};
...
namespace TestClassTypeProps {
    ...
    const extraProps: ExtraPropsHandlers<TContext, TSource> = {
        prop1: (
            ctx: TContext,
            parent: unknown,
            child: SxiObject<TContext, typeof TestSource>,
            value: string,
        ) => {},
    };
